...
<?php
 ...
 $pocode = base64_decode($_GET['code']);
    function  getdata(){
        ...
        global $pocode;
        $query = mysqli_query($dbcon, "SELECT * FROM purchaseorder,clients,companies,contacts WHERE ponumber = '$pocode' && poclient = clientcode  && pocompany LIKE compno ");
...
class PDF extends FPDF
{
    ...
    function Header()
    {
        ...
        global $pocode;
        $data = getdata();
        $this->Image('images/hpventureslogo.jpg',15,28,40,22);
        ...
        $this->SetFont('Arial', '', 9);
        $this->Cell(25, 5, 'Address:', 1, 0);
        $this->Cell(120, 5, $data['compaddr'], 1, 0);//input
        $this->Cell(20, 5, 'Tel #', 1, 0, 'R');
        $this->Cell(25, 5, $data['comptelno'], 1, 1);//input

        $this->SetFont('Arial', 'B', 8);
        $this->SetTextColor(255, 255, 255);

        $this->Cell(190, 115, '', 1, 0);
        $this->Cell(0, 0, '', 0, 1);
...
while($ctr <= 20){
    ...
    {
        $pdf->Cell(25,5,$dataitem['poitemname'],1,0,'C');
        $pdf->Cell(25,5,$dataitem['poitemquantity'],1,0,'C');
        $pdf->Cell(95,5,$dataitem['poitemdesc'],1,0,'C');
        $pdf->Cell(20,5,$dataitem['poitemprice'],1,0,'C');
        $pdf->Cell(25,5,$dataitem['poitemtotal'],1,1,'C');
        $ctr++;
    }
...
$pdf->Cell(25,5,$data['pototal'],1,1,'C');//input
/*$pdf->AddPage();*/
ob_end_clean();
$pdf->Output();
